  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 md:p-8 bg-white text-black rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden">
      ...
      <nav
        ...
        className="flex items-center text-xs md:text-sm text-gray-500 mb-4 truncate"
      >
        {breadcrumbs.map((item, index) => (
          <React.Fragment key={index}>
            <a href={item.href} className="hover:text-black transition-colors whitespace-nowrap">
              {item.label}
            </a>
      ...
      <div className="absolute top-6 right-6 md:top-8 md:right-8 z-10">
        <div className="flex items-center gap-1 bg-white/90 backdrop-blur-md p-0.5 rounded-full shadow-sm border border-gray-100">
          <Button
            variant="ghost"
            size="icon"
            className="h-7 w-7 rounded-full text-gray-500 hover:text-black"
          >
      ...
      <main className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 relative z-0">
        ...
        <div className="flex flex-col">
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight leading-tight pr-12">
            {product.name}
          </h1>
          <div className="mt-4">
          ...
          <div className="flex flex-col my-6">
            <Button size="lg" className="w-full gap-2 bg-black text-white hover:bg-gray-800">
            ...
            {product.tags.map((tag, index) => {
              ...
              return (
                <Badge
                  key={index}
                  variant="secondary"
                  className="text-xs font-medium py-1 px-3 gap-1.5 bg-gray-100 text-gray-700 hover:bg-gray-200"
                >
          ...
          <p className="text-gray-600 text-sm leading-relaxed">
            {product.description}
            <a href="#" className="text-black font-medium hover:underline ml-2">
              Read more
            </a>
          </p>
          ...
          <div className="mt-8 p-4 bg-white rounded-2xl border border-gray-100 hover:bg-gray-50/50 transition-colors shadow-sm w-full">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  <p className="font-bold text-[15px] text-gray-900">{seller.name}</p>
              ...
              <div className="flex items-center gap-4">
                <Button
                  variant="outline"
                  size="sm"
                  className="gap-2 border-gray-200 bg-white text-black hover:bg-gray-50 h-9 rounded-full px-5 text-sm font-semibold shadow-sm"
                >
                  <Send className="h-3.5 w-3.5" /> Contact
                </Button>
                <Button
                  variant="link"
                  className="text-gray-500 hover:text-gray-900 text-[11px] font-bold uppercase tracking-widest p-0 h-auto md:hidden"
                >
                  All listings &rarr;
                </Button>
              </div>
